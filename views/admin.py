...
from db import db, Utilisateur, Parametre, Objet, Armoire, Categorie, Fournisseur, Kit, KitObjet, Budget, Depense, Echeance, Historique, Etablissement, Reservation, Suggestion, Salle
...
def generer_budget_pdf_pro(data_export, metadata):
    ...
    style_etab = ParagraphStyle('Etab', parent=styles['Normal'], fontSize=18, textColor=SCIENTRAL_BLUE, fontName='Helvetica-Bold', alignment=2)
    style_sous = ParagraphStyle('Sous', parent=styles['Normal'], fontSize=8, textColor=colors.grey, fontName='Helvetica-Oblique', alignment=2)
    if logo_image_data:
        ...
        elements.append(Paragraph(metadata['etablissement'], style_etab))
...
def ajouter_salle():
    ...
    try:
        salle = Salle(nom=nom, description=description or None, capacite=int(capacite) if capacite.isdigit() else None, etablissement_id=etablissement_id)
...
def supprimer_salle(salle_id):
    ...
    if not salle or salle.etablissement_id != etablissement_id:
        ...
        return redirect(url_for("admin.gestion_salles"))
    try:
        db.session.execute(db.update(Reservation).where(Reservation.salle_id == salle_id).values(salle_id=None))
