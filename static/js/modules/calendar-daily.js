...
function renderEvents(config) {
    ...
    // Dédupliquer par groupe_id
    const uniqueGroupes = {};
    reservations.forEach(r => { uniqueGroupes[r.groupe_id] = r; });
    const events = Object.values(uniqueGroupes).map(resa => {
        const [dH, dM] = resa.debut.split(':').map(Number);
        const [fH, fM] = resa.fin.split(':').map(Number);
        return {
            ...resa,
            startMin: Math.max(dH * 60 + dM, config.startTotal),
            endMin: Math.min(fH * 60 + fM, config.endTotal)
        };
    }).filter(e => e.startMin < e.endMin);

    // Algorithme de colonnes pour éviter les chevauchements
    const columns = [];
    events.forEach(event => {
        ...
        for (let col of columns) {
            ...
            if (last.endMin <= event.startMin) {
                col.push(event);
                event._col = columns.indexOf(col);
                placed = true;
                break;
            }
        }
    ...
    events.forEach(resa => {
        const topPx = ((resa.startMin - config.startTotal) / 60) * slotHeight;
        const heightPx = ((resa.endMin - resa.startMin) / 60) * slotHeight;
        const col = resa._col || 0;
...
function initTooltip() {
    ...
    if (deleteModalEl) {
        ...
        if (confirmDeleteBtn) {
            confirmDeleteBtn.addEventListener('click', () => {
                if (groupIdToDelete) {
                    const radioAll = deleteModalEl.querySelector('input[name="deleteScope"][value="all"]');
                    const supprimerTout = radioAll ? radioAll.checked : false;
                    performDeletion(groupIdToDelete, supprimerTout);
                }
            });
        }
    }
            ...
            if (details.can_edit) {
                ...
                if (deleteBtn) {
                    deleteBtn.addEventListener('click', function() {
                        ...
                        const hasRecurrence = this.dataset.recurrenceId && this.dataset.recurrenceId !== 'null';
                        if (hasRecurrence && deleteModalInstance) {
                            // Afficher choix suppression
                            const modalBody = deleteModalEl.querySelector('.modal-body');
                            if (modalBody) {
                                modalBody.innerHTML = `
                                    <p class="mb-3 fs-5">Voulez-vous vraiment supprimer cette réservation ?</p>
                                    <p class="text-muted small mt-2">Cette réservation fait partie d'une série récurrente.</p>
                                    <div class="d-flex flex-column gap-2 mt-3">
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="deleteScope" id="deleteOne" value="one" checked>
                                            <label class="form-check-label" for="deleteOne">Supprimer uniquement cette occurrence</label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="deleteScope" id="deleteAll" value="all">
                                            <label class="form-check-label text-danger fw-bold" for="deleteAll">Supprimer toutes les occurrences de la série</label>
                                        </div>
                                    </div>`;
                            }
                            deleteModalInstance.show();
                        } else if (deleteModalInstance) {
                            // Réinitialiser le body par défaut
                            const modalBody = deleteModalEl.querySelector('.modal-body');
                            if (modalBody) {
                                modalBody.innerHTML = '<p class="mb-0 fs-5">Voulez-vous vraiment supprimer cette réservation ?</p><p class="text-muted small mt-2">Cette action est irréversible.</p>';
                            }
                            deleteModalInstance.show();
                        } else if (confirm("Supprimer ?")) performDeletion(groupIdToDelete, false);
                    });
                }
            }
        } catch (error) {
    ...
    function performDeletion(groupId, supprimerTout = false) {
        ...
        fetch('/api/supprimer_reservation', {
            ...
            body: JSON.stringify({ groupe_id: groupId, supprimer_tout: supprimerTout })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
